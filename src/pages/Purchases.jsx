...
export default function Purchases() {
  ...
  const save = async (e) => {
    ...
    if (!supplierId || valid.length === 0) { alert('Select a supplier and at least one item.'); return }
    ...
    await runTransaction(db, async (tx) => {
      ...
      valid.forEach((i, idx) => {
        ...
        tx.update(productRefs[idx], { stock_qty: (productSnaps[idx].data()?.stock_qty || 0) + Number(i.qty) })
      ...
      if (total - paid > 0) {
        ...
        tx.update(supplierRef, { balance_owed: (supplierSnap.data()?.balance_owed || 0) + (total - paid) })
      }
    })
