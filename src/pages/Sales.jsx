...
import { celebrateSale } from '../utils/celebrate'
...
export default function Sales() {
  ...
  const POINT_VALUE = 100 // 1 loyalty point = 100 currency units when redeemed
  const POINTS_EARN_RATE = 10000 // 1 point earned per 10,000 spent

  const selectedCustomer = customers.find((c) => c.id === customerId)
  const maxRedeemable = selectedCustomer?.loyalty_points || 0
  ...
  const resetSaleForm = () => {
    ...
    setNotes(''); setNewCustomerName(''); setNewCustomerPhone(''); setRedeemPoints('0')
  }
            ...
            {cart.map((i) => {
              ...
              return (
                <div key={i.product.id} className="border border-gray-100 dark:border-gray-800 rounded-lg p-2">
                  ...
                  <input placeholder="Serial / IMEI (optional — e.g. for phones)" className="mt-1 w-full text-xs px-2 py-1 rounded border border-gray-200 dark:border-gray-700 dark:bg-gray-800" value={i.serial} onChange={(e) => updateSerial(i.product.id, e.target.value)} />
                </div>
              )
            })}
          </div>
      ...
      <Modal open={confirmOpen} onClose={() => setConfirmOpen(false)} title="Confirm Sale Details" wide>
        <div className="space-y-3">
          ...
          {selectedCustomer && maxRedeemable > 0 && (
            <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800 rounded-lg p-2">
              <p className="text-xs text-emerald-700 dark:text-emerald-400 mb-1">
                {selectedCustomer.name} has <strong>{maxRedeemable} loyalty points</strong> ({formatMoney(maxRedeemable * POINT_VALUE, company.currency)} value)
              </p>
              <Input label={`Redeem Points (max ${maxRedeemable})`} type="number" min="0" max={maxRedeemable} value={redeemPoints} onChange={(e) => setRedeemPoints(Math.min(Number(e.target.value) || 0, maxRedeemable))} className="!mb-0" />
            </div>
          )}
          ...
          <div className="border-t border-gray-100 dark:border-gray-800 pt-3 space-y-1 text-sm">
            <div className="flex justify-between text-gray-500"><span>Subtotal</span><span>{formatMoney(subtotal, company.currency)}</span></div>
            {Number(discount) > 0 && <div className="flex justify-between text-gray-500"><span>Discount</span><span>-{formatMoney(Number(discount), company.currency)}</span></div>}
            {redeemDiscount > 0 && <div className="flex justify-between text-emerald-600"><span>Loyalty Points Redeemed</span><span>-{formatMoney(redeemDiscount, company.currency)}</span></div>}
            <div className="flex justify-between font-bold text-base text-gray-800 dark:text-gray-100"><span>Total</span><span>{formatMoney(total, company.currency)}</span></div>
