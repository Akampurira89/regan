...
export default function Sales() {
  ...
  const taxAmt = company.tax_enabled ? cart.reduce((s, i) => s + (i.rate * i.qty * (i.product.tax_rate || 0)) / 100, 0) : 0
  ...
  const resetSaleForm = () => {
    ...
    setNotes(''); setNewCustomerMode(false); setNewCustomerName(''); setNewCustomerPhone('')
  }

  const checkout = async () => {
    if (cart.length === 0) return
    if (isCredit && !customerId && !newCustomerMode) { alert('Credit sales require a customer.'); return }
    ...
    try {
      ...
      if (newCustomerMode && newCustomerName) {
        const custRef = await addDoc(collection(db, 'customers'), { name: newCustomerName, phone: newCustomerPhone, loyalty_points: 0, created_at: serverTimestamp() })
        finalCustomerId = custRef.id
        finalCustomer = { id: custRef.id, name: newCustomerName, phone: newCustomerPhone }
      }
      ...
      const saleData = {
        ...
        subtotal, discount: discountAmt, tax_amount: taxAmt, total,
      ...
      // Firestore transaction: create the sale + sale items + decrement stock for
      // every product atomically. This is the equivalent of the Postgres trigger
      // that reduced stock automatically on the Supabase version.
      const saleRef = doc(collection(db, 'sales'))
      await runTransaction(db, async (tx) => {
        // Re-read product stock inside the transaction to avoid overselling if two
        // cashiers check out the same item at the same time.
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 max-h-[65vh] overflow-y-auto scrollbar-thin pr-1">
          {filteredProducts.map((p) => (
            <button
              ...
              className="text-left bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-xl p-3 hover:border-blue-400 disabled:opacity-40 disabled:cursor-not-allowed transition"
            >
              ...
              <div className="flex items-center justify-between mt-2">
                <span className="text-sm font-bold text-blue-600">{formatMoney(p.selling_price, company.currency)}</span>
      ...
      <Card title="Current Sale" actions={<ShoppingCart size={16} className="text-gray-400" />} className="h-fit lg:sticky lg:top-20">
            ...
            {cart.map((i) => (
              <div key={i.product.id} className="border border-gray-100 dark:border-gray-800 rounded-lg p-2">
                ...
                <div className="flex items-center justify-between mt-1">
                  ...
                  {company.allow_price_negotiation ? (
                    <div className="flex items-center gap-1">
                      <span className="text-xs text-gray-400">@</span>
                      <input
                        type="number"
                        className="w-20 text-sm text-right px-1 py-0.5 rounded border border-gray-200 dark:border-gray-700 dark:bg-gray-800"
                        value={i.rate}
                        onChange={(e) => updateRate(i.product.id, e.target.value)}
                      />
                    </div>
                  ) : (
                ...
                {company.allow_price_negotiation && i.rate !== i.product.selling_price && (
                  <p className="text-[0.7rem] text-amber-600 mt-0.5">
                    Negotiated price ({i.rate < i.product.selling_price ? 'below' : 'above'} list price {formatMoney(i.product.selling_price, company.currency)})
                    {i.rate < i.product.buying_price && <span className="text-red-600 font-medium"> — below cost!</span>}
                  </p>
                )}
                {company.allow_price_negotiation && (
                  <p className="text-right text-xs text-gray-400">Line total: {formatMoney(i.rate * i.qty, company.currency)}</p>
                )}
        ...
        {!newCustomerMode ? (
          ...
          <div className="grid grid-cols-2 gap-2">
            <Input label="New Customer Name" value={newCustomerName} onChange={(e) => setNewCustomerName(e.target.value)} />
            <Input label="Phone" value={newCustomerPhone} onChange={(e) => setNewCustomerPhone(e.target.value)} />
          </div>
        )}
        <button className="text-xs text-blue-600 -mt-2 mb-3" onClick={() => setNewCustomerMode(!newCustomerMode)}>
          {newCustomerMode ? 'Choose existing customer instead' : '+ New customer'}
        </button>
        ...
        <div className="border-t border-gray-100 dark:border-gray-800 pt-3 mt-2 space-y-1 text-sm">
          ...
          {company.tax_enabled && <div className="flex justify-between text-gray-500"><span>Tax</span><span>{formatMoney(taxAmt, company.currency)}</span></div>}
        ...
        {(paymentMethod === 'mtn_momo' || paymentMethod === 'airtel_money') && (
          <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 rounded-lg p-2 mt-2 text-sm text-center">
        ...
        <Button className="w-full mt-3" disabled={cart.length === 0 || processing} onClick={checkout}>
          {processing ? 'Processing...' : `Charge ${formatMoney(total, company.currency)}`}
        </Button>
      </Card>
