      ...
      <div className="grid lg:grid-cols-2 gap-4">
        <Card title="Business Settings">
          <form onSubmit={submit}>
            ...
            <Checkbox label="Change the app's colour automatically each time it loads (fun mode!)" checked={form.auto_color_rotate} onChange={(e) => setForm({ ...form, auto_color_rotate: e.target.checked })} />
            <label className="block text-sm mb-3">
              <span className="block mb-1 text-gray-600 dark:text-gray-300 font-medium">Fixed Accent Color {form.auto_color_rotate && '(disabled while auto-colour is on)'}</span>
              <div className="flex items-center gap-2">
                <input type="color" disabled={form.auto_color_rotate} value={form.accent_color || '#c2410c'} onChange={(e) => setForm({ ...form, accent_color: e.target.value })} className="w-10 h-10 rounded cursor-pointer border border-gray-300 dark:border-gray-700 disabled:opacity-40 disabled:cursor-not-allowed" />
