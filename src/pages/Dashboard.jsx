...
export default function Dashboard() {
  ...
  const [stats, setStats] = useState({ todaySales: 0, todayCount: 0, lowStock: 0, openDebts: 0, owedToConsignors: 0 })
  ...
  useEffect(() => {
    (async () => {
      ...
      const owedToConsignors = consignSnap.docs.reduce((s, d) => s + Number(d.data().owner_amount || 0), 0)
      const customersMap = Object.fromEntries(customersSnap.docs.map((d) => [d.id, d.data()]))

      setStats({
        todaySales, todayCount: todaySalesDocs.length,
        lowStock: lowStock.length, openDebts, owedToConsignors,
      })
      ...
      // Show the low-stock nudge at most once per day, so it doesn't nag every visit.
      if (lowStock.length > 0 && localStorage.getItem(NUDGE_KEY) !== todayStr()) {
        setShowNudge(true)
      }
    })()
  }, [])
  ...
  const sendLowStockAlert = () => {
    const list = lowStockItems.map((p) => `- ${p.name} (${p.stock_qty} left)`).join('\n')
    const message = `${template.shop_name} — Low Stock Alert (${new Date().toLocaleDateString('en-GB')}):\n${list}${stats.lowStock > lowStockItems.length ? `\n...and ${stats.lowStock - lowStockItems.length} more` : ''}`
    openWhatsApp('', message)
    dismissNudge()
  }

  const sendDailySummary = () => {
    const message = `${template.shop_name} — Daily Summary (${new Date().toLocaleDateString('en-GB')}):\nSales: ${formatMoney(stats.todaySales, company.currency)}\nTransactions: ${stats.todayCount}\nOutstanding debts: ${formatMoney(stats.openDebts, company.currency)}${stats.lowStock > 0 ? `\nLow stock items: ${stats.lowStock}` : ''}`
    openWhatsApp('', message)
  }
    ...
    <div className="space-y-6">
      <div className="rounded-xl bg-gradient-to-r from-brand to-brand-dark text-white p-5">
        <div className="flex items-start justify-between gap-3 flex-wrap">
          ...
          <button onClick={sendDailySummary} className="bg-white/15 hover:bg-white/25 transition rounded-lg px-3 py-2 text-sm font-medium flex items-center gap-1.5">
            <MessageCircle size={15} /> Send Today's Summary
          </button>
        </div>
      </div>
      ...
      {showNudge && (
        <div className="flex items-start gap-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl p-3">
          ...
          <div className="flex-1">
            <p className="text-sm font-medium text-amber-800 dark:text-amber-300">{stats.lowStock} item{stats.lowStock !== 1 ? 's are' : ' is'} running low on stock.</p>
            <p className="text-xs text-amber-700/80 dark:text-amber-400/80">{lowStockItems.slice(0, 3).map((p) => p.name).join(', ')}{stats.lowStock > 3 ? ', and more...' : ''}</p>
          </div>
          <Button variant="success" onClick={sendLowStockAlert}><MessageCircle size={14} className="inline mr-1" />Alert via WhatsApp</Button>
          <button onClick={dismissNudge} className="text-amber-600 p-1"><X size={16} /></button>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <StatCard label="Owed to Consignors" value={formatMoney(stats.owedToConsignors, company.currency)} icon={HandCoins} color="purple" />
      </div>
