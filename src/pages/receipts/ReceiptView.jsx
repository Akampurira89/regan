...
export default function ReceiptView({ sale, items, customer, template, onClose }) {
  ...
  const handleDownloadPDF = () => {
    ...
    doc.text(`${template.customer_contact_label}: ${customer?.name || 'Walk-in'}`, 5, y); y += 6
    ...
    if (template.show_footer_contact_line) {
      const contactLabel = template.customer_contact_label === 'M/S' ? "Customer's Contact" : 'Contact'
      doc.text(customer?.phone ? `${contactLabel}: ${customer.phone}` : `${contactLabel}.........................`, 5, finalY)
    }
  ...
  return (
    <div className="font-serif">
      ...
      <div className="text-sm border-b border-dotted border-black pb-1 mb-2">
        <strong>{template.customer_contact_label || 'M/S'}</strong> {customer?.name || 'Walk-in'}
      </div>
      ...
      <table className="w-full mt-1 border border-black border-collapse">
        <thead>
          <tr className="border-b border-black">
            ...
            <th className="text-right py-1 px-1 border-r border-black w-16 receipt-num-col">{cols.rate}</th>
            <th className="text-right py-1 px-1 w-20 receipt-num-col">{cols.amount}</th>
          </tr>
        </thead>
        <tbody>
          {(items || []).map((i, idx) => (
            <tr key={idx} className="border-b border-black/20">
              ...
              <td className="py-0.5 px-1 border-r border-black align-top text-right receipt-num-col">{formatMoney(i.rate)}</td>
              <td className="py-0.5 px-1 align-top text-right receipt-num-col">{formatMoney(i.amount)}</td>
            </tr>
          ))}
        </tbody>
        <tfoot>
      ...
      {(template.show_discount && sale.discount > 0) || sale.balance_due > 0 ? (
      ...
      {template.show_footer_contact_line && (
        <p className="mt-2 text-sm">
          {template.customer_contact_label === 'M/S' ? "Customer's Contact" : 'Contact'}
          {customer?.phone ? (
            <span className="font-medium">: {customer.phone}</span>
          ) : (
            <span className="border-b border-dotted border-black">.........................................</span>
          )}
        </p>
      )}
    </div>
